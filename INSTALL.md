...
cmux-surface-skills (`suf` v5 포함) 를 새로운 macOS 머신에 처음 설치하는 절차.
...
| **cmux** | suf/cmux 스킬에 필수 | `cmux version` |
...
```bash
ls ~/.agents/skills/suf/scripts/
# 다음 두 파일이 보여야 함:
#   suf-lib.sh         (v3/v4 fallback)
#   suf-v5-lib.sh      (v5, 권장)
```
...
ln -sfn "../../.agents/skills/suf" "$HOME/.claude/skills/suf"
```
...
```bash
ls -la ~/.claude/skills/suf
# 출력에 화살표 (->) 가 있으면 심볼릭 OK
# 예: suf -> ../../.agents/skills/suf
```
...
ln -sfn "../../.agents/skills/suf" "$HOME/.gemini/skills/suf"
```

(Gemini 가 스킬 자동 로드 안 하면 GEMINI.md 에 `@~/.agents/skills/suf/SKILL.md` 추가)

### 2c. Junie (JetBrains)

```bash
mkdir -p ~/.junie/skills
ln -sfn "../../.agents/skills/suf" "$HOME/.junie/skills/suf"
```
...
bash -c '
  ...
  echo "=== 1) public 함수 확인 ==="
  declare -F | grep -E "suf_(ask|other)"
  # 다음 3개 보여야 함:
  #   suf_ask
  #   suf_ask_unsafe
  #   suf_other_surfaces
  ...
  echo "=== 2) cap 기본값 확인 ==="
  echo "PROMPT_MAX=$SUF_V5_PROMPT_MAX (기본 500)"
  echo "RESPONSE_MAX=$SUF_V5_RESPONSE_MAX (기본 4096)"
  echo "TIMEOUT=$SUF_V5_TIMEOUT (기본 30)"

  echo "=== 3) cap 강제 reject 확인 ==="
  long=$(printf "a%.0s" $(seq 1 600))
  suf_ask surface:1 "$long" 2>&1 >/dev/null
...
source ~/.agents/skills/suf/scripts/suf-v5-lib.sh
suf_ask surface:9 "현재 한국 시간만 한 줄로"
```
...
### `[suf v5] cannot auto-detect sidecar on surface:X`

sidecar 의 프로세스가 화이트리스트 (claude/codex/gemini/zsh/bash 등) 에 없음. 명시 mode 로 우회:

```bash
suf_ask surface:X "..." --mode llm     # LLM TUI 면
suf_ask surface:X "date" --mode worker # shell 이면
```
...
3. **sidecar 가 마커 / fifo 안내문을 무시함** — `SUF_V5_AUTO_CAP=on` 인지 확인
...
```bash
suf_ask_unsafe surface:9 "$LONG" --prompt-max 4000 --response-max 65536
```
...
```bash
# 더 엄격한 cap 원할 때
export SUF_V5_PROMPT_MAX=300
export SUF_V5_RESPONSE_MAX=2048
export SUF_V5_TIMEOUT=20

# 자동 cap 안내문 끄고 raw prompt 만 보내기 (디버깅)
export SUF_V5_AUTO_CAP=off
...
심볼릭은 그대로라 변경 자동 반영. `suf-v5-lib.sh` 만 바뀌면 호출자가 `source` 만 새로 하면 즉시 적용. SKILL.md 가 바뀌면 LLM CLI 재시작 후 새 description 반영.
...
```bash
# 심볼릭 제거
rm -f ~/.claude/skills/suf ~/.gemini/skills/suf ~/.junie/skills/suf

# codex AGENTS.md 의 suf 라인 수동 삭제
